...
object Capacity {
  def subtract (c1: Capacity, c2: Capacity): Capacity = Capacity(c1.value - c2.value)
  def add (c1: Capacity, c2: Capacity): Capacity = Capacity(c1.value + c2.value)
}

case class MachineId(value: Int) extends AnyVal

object MachineId {
  val missing = MachineId(-1)
}
  
case class AppId(value: Int) extends AnyVal


class DCLoadBalancer2() {
  ...
  def stopApplication(appId: AppId): Unit = {
    appLocation.get(appId) match {
      case None => ()
      case Some(RunningApp(ladUse, machineId)) =>
        appLocation -= appId
        machineCapacities(machineId) = Capacity.add(machineCapacities(machineId), ladUse)
    }
  }
  ...
  private def findAllocatableMachine(loadUse: Capacity): Option[(MachineId, Capacity)] = 
    ...
    .filter { case (_, cap) => cap.value >= loadUse.value }
    .minByOption { case (id, cap) => (-cap.value, id.value) }
    .map { case (id, cap) => (id, Capacity.subtract(cap, loadUse)) }

  private def getRunningApps(machineId: MachineId): SeqMap[AppId, RunningApp] =
    appLocation.filter { case (appId, app) => 
      app.machineId == machineId 
    }
}


@main def mainLoadDistributor2 =
  ...
  var loadBalancer: DCLoadBalancer2 = null

  def toOut (r : Unit | MachineId | List[AppId]): Output = r match {
    case () => Output.Null
    case MachineId(value) => Output.Number(value)
    case list: List[AppId] => Output.Array(list.map(_.value))
  }

  input.zip(data).zip(expected).foreach { case ((i, d), e) =>
    val out = i match
      case Instructions.DCLoadBalancer  => toOut { loadBalancer = DCLoadBalancer2() }
      case Instructions.addMachine      => toOut(loadBalancer.addMachine(MachineId(d(0)), Capacity(d(1))))
      case Instructions.addApplication  => toOut(loadBalancer.addApplication(AppId(d(0)), Capacity(d(1))))
      case Instructions.getApplications => toOut(loadBalancer.getApplications(MachineId(d(0))))
      case Instructions.stopApplication => toOut(loadBalancer.stopApplication(AppId(d(0))))
      case Instructions.removeMachine   => toOut(loadBalancer.removeMachine(MachineId(d(0))))
